...
#if defined(NDEBUG)
# error "fef cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "fef Signed Message:\n";
                        ...
                        bool* pfMissingInputs)
{
    ...
    // fef: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// fef: find block wanted by given orphan block
...
{
    int64_t nSubsidy = (35970099 * COIN)/Params().LastPOWBlock();
...
// fef: find last block index up to pindex
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    // fef: target change every block
    // fef: retarget with exponential moving toward target spacing
    CBigNum bnNew;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    // fMiner is true when called from the internal fef miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // fef: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // fef: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // fef: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // fef: track money supply and mint amount info
...
// fef: total coin age spent in transaction, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // fef: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // fef: compute stake entropy bit for stake modifier
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom) {
            ...
            // fef: getblocks may not obtain the ancestor block rejected
...
#ifdef ENABLE_WALLET
// fef: attempt to generate suitable proof-of-stake
